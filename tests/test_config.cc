...
// 创建默认配置项
auto config_system_port = xuan_web::Config::Lookup<int>("system.port", 6666);
auto config_test_list = xuan_web::Config::Lookup<std::vector<std::string>>(
    "test_list", std::vector<std::string>{"vector", "string"});
auto config_test_linklist = xuan_web::Config::Lookup<std::list<std::string>>(
    "test_linklist", std::list<std::string>{"list", "string"});
auto config_test_map = xuan_web::Config::Lookup<std::map<std::string, std::string>>(
    "test_map", std::map<std::string, std::string>{
                    std::make_pair("map1", "srting"),
                    std::make_pair("map2", "srting"),
                    std::make_pair("map3", "srting")});
auto config_test_set = xuan_web::Config::Lookup<std::set<int>>(
...
namespace xuan_web
{
// xuan_web::LexicalCast 针对自定义类型的偏特化
template <>
...
auto config_test_user_type = xuan_web::Config::Lookup<Goods>("user.goods", Goods{});
auto config_test_uset_type_list = xuan_web::Config::Lookup<std::vector<Goods>>("user.goods_list", std::vector<Goods>{});
...
void TEST_loadConfig(const std::string& path)
{
    ...
    xuan_web::Config::LoadFromYAML(config);
}
...
void TEST_nonexistentConfig()
{
    ...
    auto log_name = xuan_web::Config::Lookup("nonexistent");
    if (!log_name)
    {
