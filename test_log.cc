...
//     auto logger = xuan_web::LoggerManager::GetInstance()->getLogger("global");
//     auto event = std::make_shared<xuan_web::LogEvent>(__FILE__, __LINE__,
//                                                  xuan_web::GetThreadID(), xuan_web::GetFiberID(), time(nullptr), "wdnmd");
//     logger->log(event);
//     logger->debug(event);
//     logger->info(event);
//     logger->warn(event);
...
void TEST_getNonexistentLogger()
{
    ...
    try
    {
        xuan_web::LoggerManager::GetInstance()->getLogger("nonexistent");
    }
...
void TEST_createLoggerByYAMLFile()
{
    ...
    auto yaml_node = YAML::LoadFile("../config.yml");
    xuan_web::Config::LoadFromYAML(yaml_node);
    auto global_logger = xuan_web::LoggerManager::GetInstance()->getGlobal();
    auto system_logger = xuan_web::LoggerManager::GetInstance()->getLogger("system");
    ...
    // auto event = MAKE_LOG_EVENT(xuan_web::LogLevel::DEBUG, "输出一条 debug 日志");
...
void TEST_multiThreadLog()
{
    ...
    {
        xuan_web::Thread t_1(fn_1, "thread_1");
        xuan_web::Thread t_2(fn_2, "thread_2");
    }
    sleep(10);
}
