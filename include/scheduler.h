#ifndef XUAN_WEB_SCHEDULER_H
#define XUAN_WEB_SCHEDULER_H
...
private: // 内部类
    /**
     * @brief 任务类
     * 等待分配线程执行的任务，可以是 xuan_web::Fiber 或 std::function
     * */
    struct Task
    {
...
public: // 实例方法
     ...
     * @param Executable 模板类型必须是 std::unique_ptr<xuan_web::Fiber> 或者 std::function
...
private:
    ...
    bool scheduleNonBlock(Executable&& exec, long thread_id = -1, bool instant = false)
    {
        ...
        // 创建的任务实例存在有效的 xuan_web::Fiber 或 std::function
