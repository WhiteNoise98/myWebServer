#ifndef XUAN_WEB_LOG_H
#define XUAN_WEB_LOG_H
...
#define MAKE_LOG_EVENT(level, massage) \
    std::make_shared<xuan_web::LogEvent>(__FILE__, __LINE__, xuan_web::GetThreadID(), xuan_web::GetFiberID(), ::time(nullptr), massage, level)

#define LOG_LEVEL(logger, level, massage) \
    logger->log(MAKE_LOG_EVENT(level, massage));

#define LOG_DEBUG(logger, massage) LOG_LEVEL(logger, xuan_web::LogLevel::DEBUG, massage)
#define LOG_INFO(logger, massage) LOG_LEVEL(logger, xuan_web::LogLevel::INFO, massage)
#define LOG_WARN(logger, massage) LOG_LEVEL(logger, xuan_web::LogLevel::WARN, massage)
#define LOG_ERROR(logger, massage) LOG_LEVEL(logger, xuan_web::LogLevel::ERROR, massage)
#define LOG_FATAL(logger, massage) LOG_LEVEL(logger, xuan_web::LogLevel::FATAL, massage)
...
#define LOG_FMT_DEBUG(logger, format, argv...) LOG_FMT_LEVEL(logger, xuan_web::LogLevel::DEBUG, format, argv)
#define LOG_FMT_INFO(logger, format, argv...) LOG_FMT_LEVEL(logger, xuan_web::LogLevel::INFO, format, argv)
#define LOG_FMT_WARN(logger, format, argv...) LOG_FMT_LEVEL(logger, xuan_web::LogLevel::WARN, format, argv)
#define LOG_FMT_ERROR(logger, format, argv...) LOG_FMT_LEVEL(logger, xuan_web::LogLevel::ERROR, format, argv)
#define LOG_FMT_FATAL(logger, format, argv...) LOG_FMT_LEVEL(logger, xuan_web::LogLevel::FATAL, format, argv)

#define GET_ROOT_LOGGER() xuan_web::LoggerManager::GetInstance()->getGlobal()
#define GET_LOGGER(name) xuan_web::LoggerManager::GetInstance()->getLogger(name)
...
struct LogIniter
{
    LogIniter()
    {
        auto log_config_list =
            xuan_web::Config::Lookup<std::vector<LogConfig>>("logs", {}, "日志器的配置项");
