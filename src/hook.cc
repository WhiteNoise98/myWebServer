...
static xuan_web::ConfigVar<int>::ptr g_tcp_connect_timeout =
    xuan_web::Config::Lookup("tcp.connect.timeout", 5000);
...
{
    if (!xuan_web::t_hook_enabled)
    {
        return func(fd, std::forward<Args>(args)...);
    }

    // LOG_FMT_DEBUG(xuan_web::system_logger, "doIO 代理执行系统函数 %s", hook_func_name);

    xuan_web::FileDescriptor::ptr fdp = xuan_web::FileDescriptorManager::GetInstance()->get(fd);
    if (!fdp)
    {
        return func(fd, std::forward<Args>(args)...);
    }
...
RETRY:
    ...
    if (n == -1 && errno == EAGAIN)
    {
        LOG_FMT_DEBUG(xuan_web::system_logger, "doIO(%s): 开始异步等待", hook_func_name);

        auto iom = xuan_web::IOManager::GetThis();
        xuan_web::Timer::ptr timer;
        ...
        if (timeout != static_cast<uint64_t>(-1))
        {
            timer = iom->addConditionTimer(
                ...
                [timer_info_wp, fd, iom, event](){
                    ...
                    iom->cancelEventListener(fd, static_cast<xuan_web::FDEventType>(event));
                }, 
                timer_info_wp);
        }
        int c = 0;
        ...
        int rt = iom->addEventListener(fd, static_cast<xuan_web::FDEventType>(event));
        if (rt == -1)
        {
            LOG_FMT_ERROR(xuan_web::system_logger, "%s addEventListener(%d, %u)", hook_func_name, fd, event);
            if (timer)
            {
                timer->cancel();
            }
...
unsigned int sleep(unsigned int seconds)
{
    ...
    xuan_web::Fiber::ptr fiber = xuan_web::Fiber::GetThis();
    auto iom = xuan_web::IOManager::GetThis();
...
int socket(int domain, int type, int protocol)
{
    ...
    xuan_web::FileDescriptorManager::GetInstance()->get(fd, true);
    return fd;
}
...
int connectWithTimeout(int sockfd, const struct sockaddr *addr, socklen_t addrlen, uint64_t timeout_ms)
{
    ...
    auto fdp = xuan_web::FileDescriptorManager::GetInstance()->get(sockfd);
    if (!fdp || fdp->isClosed())
    {
        errno = EBADF;
        return -1;
    ...
    */
    ...
    if (timeout_ms != static_cast<uint64_t>(-1))
    {
        timer = iom->addConditionTimer(
            ...
            [weak_timer_info, sockfd, iom](){
                ...
                t->cancelled = ETIMEDOUT;
                iom->cancelEventListener(sockfd, xuan_web::FDEventType::WRITE);
            }, 
            weak_timer_info);
    }

    int rt = iom->addEventListener(sockfd, xuan_web::FDEventType::WRITE);
    if (rt == 0)
    {
        xuan_web::Fiber::YieldToHold();
        if (timer)
    ...
    if (rt == -1)
    {
        ...
        LOG_FMT_ERROR(xuan_web::system_logger, "connectWithTimeout addEventListener(%d, write) error", sockfd);
    }
...
{
    return connectWithTimeout(sockfd, addr, addrlen, xuan_web::s_connect_timeout);
}   

int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen)
{
    int fd = doIO(sockfd, accept_f, "accept", xuan_web::FDEventType::READ, SO_RCVTIMEO, addr, addrlen);
    if (fd >= 0)
    {
        xuan_web::FileDescriptorManager::GetInstance()->get(fd, true);
    }
...
{
    return doIO(fd, read_f, "read", xuan_web::FDEventType::READ, SO_RCVTIMEO, buf, count);
}

ssize_t readv(int fd, const struct iovec *iov, int iovcnt)
{
    return doIO(fd, readv_f, "readv", xuan_web::FDEventType::READ, SO_RCVTIMEO, iov, iovcnt);
}

ssize_t recv(int sockfd, void *buf, size_t len, int flags)
{
    return doIO(sockfd, recv_f, "recv", xuan_web::FDEventType::READ, SO_RCVTIMEO, buf, len, flags);
}

ssize_t recvfrom(int sockfd, void* buf, size_t len, int flags, 
    struct sockaddr* src_addr, socklen_t* addrlen)
{
    return doIO(sockfd, recvfrom_f, "recvfrom", xuan_web::FDEventType::READ, SO_RCVTIMEO,
        buf, len, flags, src_addr, addrlen);
}

ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags)
{
    return doIO(sockfd, recvmsg_f, "recvfrom", xuan_web::FDEventType::READ, SO_RCVTIMEO, msg, flags);
}

ssize_t write(int fd, const void *buf, size_t count)
{
    return doIO(fd, write_f, "write", xuan_web::FDEventType::WRITE, SO_SNDTIMEO, buf, count);
}

ssize_t writev(int fd, const struct iovec *iov, int iovcnt)
{
    return doIO(fd, writev_f, "writev_f", xuan_web::FDEventType::WRITE, SO_SNDTIMEO, iov, iovcnt);
}

ssize_t send(int sockfd, const void *buf, size_t len, int flags)
{
    return doIO(sockfd, send_f, "send", xuan_web::FDEventType::WRITE, SO_SNDTIMEO, buf, len, flags);
}

ssize_t sendto(int sockfd, const void *buf, size_t len, int flags,
    const struct sockaddr *dest_addr, socklen_t addrlen)
{
    return doIO(sockfd, sendto_f, "sendto", xuan_web::FDEventType::WRITE, SO_SNDTIMEO, buf, len, flags, dest_addr, addrlen);
}

ssize_t sendmsg(int sockfd, const struct msghdr *msg, int flags)
{
    return doIO(sockfd, sendmsg_f, "sendmsg", xuan_web::FDEventType::WRITE, SO_SNDTIMEO, msg, flags);
}
...
int close(int fd)
{
    ...
    if (fdp)
    {
        ...
        xuan_web::FileDescriptorManager::GetInstance()->remove(fd);
    }
    return close_f(fd);
} 
...
int fcntl(int fd, int cmd, ... /* arg */ )
{
    ...
    switch (cmd)
    {
        case F_SETFL:
        {
            ...
            va_end(va);
            auto fdp = xuan_web::FileDescriptorManager::GetInstance()->get(fd);
