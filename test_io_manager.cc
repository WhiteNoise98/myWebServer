...
xuan_web::Logger::ptr g_logger = GET_ROOT_LOGGER();
...
void TEST_CreateIOManager()
{
    ...
    else
    {
        ...
        iom.addEventListener(sockfd, xuan_web::FDEventType::READ, [&]() {
            recv(sockfd, buffer, sizeof(buffer), 0);
            LOG_FMT_INFO(g_logger, "服务端回应: %s", buffer);
            iom.cancelAll(sockfd);
            close(sockfd);
        });
        iom.addEventListener(sockfd, xuan_web::FDEventType::WRITE, [&]() {
